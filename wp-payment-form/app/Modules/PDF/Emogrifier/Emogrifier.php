...
namespace WPPayForm\App\Modules\PDF\Emogrifier;
/**
...
class Emogrifier
{
    ...
    private function clearCache($key)
    {
        ...
        if (!in_array($key, $allowedCacheKeys, true)) {
            throw new \InvalidArgumentException('Invalid cache key: ' . esc_textarea($key), 1391822035);
        }
    ...
    )
    {
        ...
        if ($selector !== '') {
            throw new \InvalidArgumentException(
                sprintf('%1$s in selector >> %2$s << in %3$s on line %4$u', esc_html($message), esc_html($selector), esc_html($file), esc_html($line)),
                1509279985
            );
        }
        ...
        if (isset($context['xPath'])) {
            throw new \RuntimeException(
                sprintf('%1$s in %2$s on line %3$u', esc_html($message), esc_html($file), esc_html($line)),
                1509280067
            );
        }
