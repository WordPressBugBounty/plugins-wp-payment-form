...
class ApiRequest
{
     ...
     * @return string
     */
    public static function get_secret_key($formId = false)
    {
        if (!self::$secret_key) {
            $stripe = new Stripe();
            self::set_secret_key($stripe->getSecretKey($formId));
        }
    ...
    public static function get_user_agent()
    {
        ...
        // function_exists check is added to support shared hosting where some functions are disabled.
    ...
    public static function request($request, $api = 'charges', $method = 'POST')
    {
        ...
        if ('charges' === $api && 'POST' === $method) {
            ...
            $idempotency_key = apply_filters('wppayform/stripe_idempotency_key', Arr::get($request, 'metadata.wppayform_tid') . '-' . $source . '-' . $api, $request);
            $headers['Idempotency-Key'] = $idempotency_key;
        }
        // dd(self::ENDPOINT . $api, $request);
        ...
        if (is_wp_error($response) || empty($response['body'])) {
            return new \WP_Error('stripe_error', __('There was a problem connecting to the Stripe API endpoint.', 'wp-payment-form'));
        }
